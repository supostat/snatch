...
#[tauri::command]
pub async fn open_path(path: String, state: State<'_, AppState>) -> Result<(), AppError> {
    let allowed_dirs = build_allowed_dirs();
    let _safe = SafePath::new(&path, &allowed_dirs)?;

    let result =
        opener::open(&path).map_err(|e| AppError::Dialog(format!("failed to open path: {e}")));
...
pub async fn open_url(url: String) -> Result<(), AppError> {
    if !url.starts_with("https://") {
        return Err(AppError::Dialog("only HTTPS URLs are allowed".to_string()));
    }

    opener::open(&url).map_err(|e| AppError::Dialog(format!("failed to open URL: {e}")))?;

    Ok(())
}

#[tauri::command]
pub async fn show_in_folder(path: String, state: State<'_, AppState>) -> Result<(), AppError> {
    let allowed_dirs = build_allowed_dirs();
    let _safe = SafePath::new(&path, &allowed_dirs)?;
